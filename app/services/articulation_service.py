...
class ArticulationService:
    ...
    @staticmethod
    def build_course_mappings(db: Session, university_id: int, college_id: int):
    ...
    def process_expression(expression, course_mappings):
        ...
        for group in groups:
            if isinstance(group, dict) and "operator" in group:
                ...
                processed_groups.append(ArticulationService.process_expression(group, course_mappings))
            else:
    ...
    @staticmethod
    def build_requirement_tree(articulation_groups, course_mappings):
        ...
        if len(articulation_groups) == 1:
            ...
            return ArticulationService.process_expression(main_group.expression, course_mappings)
        else:
            ...
            return ArticulationService.process_expression(combined_expression, course_mappings)
    ...
    def process_expression_tree(db, node, uni_to_cc_map, cc_to_uni_map):
        ...
        if node.node_type == NodeType.COURSE:
            if node.university_course:
                ArticulationService.map_course_articulations(db, node, uni_to_cc_map, cc_to_uni_map)
        elif node.node_type == NodeType.OPERATOR:
            for child in node.child_nodes:
                ArticulationService.process_expression_tree(db, child, uni_to_cc_map, cc_to_uni_map)
    
    @staticmethod
    def map_course_articulations(db, node, uni_to_cc_map, cc_to_uni_map):
    ...
    @staticmethod
    def process_map_uni_and_cc(db, articulation_groups, uni_to_cc_map, cc_to_uni_map):
        ...
        for group in articulation_groups:
            ...
            if group.root_node:
                ArticulationService.process_expression_tree(db, group.root_node, uni_to_cc_map, cc_to_uni_map)
    
    @staticmethod
    def get_courses_to_take(uni_to_cc_map):
